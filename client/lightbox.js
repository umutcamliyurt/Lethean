...
export async function openTile(fileId) {
...
export function showLightbox(innerHtml, { keepMedia = false } = {}) {
  if (!keepMedia) {
    lightboxMediaList = [];
    lightboxIndex = -1;
...
export async function downloadAndSave(record, meta, existingUrl) {
  let url = existingUrl;
  try {
    if (!url) {
